...
gulp.task('copy-frontend-spec', () => gulp.src('app/src/frontend/spec/*.js')
  .pipe(gulp.dest('test/frontend/')));

gulp.task(
  'webpack',
  callback => webpack(webpackConfig).run((err, stats) => {
    if (err) {
      gutil.log('ERROR:', err);
      return callback();
    }
...
gulp.task('copy-javascript', () => gulp.src('app/src/backend/**/*.js')
  .pipe(gulp.dest('app/bin/backend')));

gulp.task('compile-typescript', () => gulp.src('app/src/backend/**/*.ts')
  .pipe(tsProject())
  .pipe(gulp.dest('app/bin/backend')));

gulp.task('copy-backend-spec', () => gulp.src('app/bin/backend/spec/*.js')
...
const testBackend = () => gulp.src('test/backend/*.spec.js')
  .pipe(jasmine({}));
