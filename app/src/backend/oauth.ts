/**
 * @module oauth
 * @description Handlers to get an access token from DNAnexus.
 */
...
import { createOAuthWindow } from './ui';
...
export const waitForCode = (
  ...
  createOAuthWindow(showInternalURL, async window => {
    if (!window) {
...
/**
 * Hooks to a browser window that waits for an access token from DNAnexus.
 *
 * @param showInternalURL Whether to use St. Jude SSO (true) or DNAnexus (false).
 * @param cb
 */
export const getToken = (
  ...
  waitForCode(showInternalURL, function(err, code) {
    if (err || !code) {
      cb(err, null);
      return;
    }
